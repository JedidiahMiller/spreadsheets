...
impl Parser {
    ...
    fn block_level_2(&mut self) -> Result<Statement, SourceCodeError> {
        ...
        if self.accept(TokenType::If)? {
            ...
            return Ok(Statement::IfElse { conditional, true_block, false_block });
        }
        if self.accept(TokenType::For)? {
            ...
            return Ok(Statement::ForEach { variable, range_start, range_end, body })
        }
        
        Err(SourceCodeError { location: vec![self.cursor], error_message: String::from("Did not find a statement") })
    }
    ...
    fn expression_level_6(&mut self) -> Result<Expression, SourceCodeError> {
        ...
        Err(SourceCodeError { location: vec![], error_message: String::from("Incomplete expression") })
    }
}
